...
class PhototourismDataset(Dataset):
    def __init__(self, root_dir, split='train', img_downscale=16, downsample=1, val_num=1, use_cache=True):
        """
        ...
        self.split = split
        assert img_downscale >= 1, 'image can only be downsampled, pleawnsampled, please set downsse set img_downscale>=1!'
    ...
    def read_meta(self):
        ...
        if self.split == 'train': # create buffer of all rays and rgb data
            print(f'Loading {len(self.img_ids_train)} {self.split} images ...')  
            ...
            if self.use_cache:
                ...
                for id_ in self.img_ids_train:
                    ...
                    self.directions = get_ray_directions(img_h, img_w, self.Ks[id_])
            ...
            pass
        # Step 6. convert np arrays to torch tensors with cuda no dictionaries
        #self.poses_dict = torch.FloatTensor(self.poses).cuda()
        #self.poses = torch.FloatTensor(self.poses).cuda()
        self.Ks = {torch.FloatTensor(K) for K in self.Ks.values()}
        #self.Ks = {id_: torch.FloatTensor(K).cuda() for id_, K in self.Ks.items()}
        #self.poses_dict = {torch.FloatTensor(p) for p in self.poses_dict.items()}
        self.nears = {id_: torch.FloatTensor([n]).cuda() for id_, n in self.nears.items()}
        self.fars = {id_: torch.FloatTensor([f]).cuda() for id_, f in self.fars.items()}
        self.xyz_world = torch.FloatTensor(self.xyz_world).cuda()
        self.xyz_world = self.xyz_world.view(-1, 3).permute(1, 0) # (3, N)
        self.xyz_world = self.xyz_world.contiguous()

        #Step 7. define img_h and img_w to self
        #self.img_h = img_h
        #self.img_w = img_w

...
class NGP(nn.Module):
    ...
    def mark_invisible_cells(self, K, poses, img_wh, chunk=64**3):
        ...
        #from poses size calculate the N_cams
        N_cams = len(poses)
        self.count_grid = torch.zeros_like(self.density_grid)
        K = ([[1.1111e+03, 0.0000e+00, 4.0000e+02],
        [0.0000e+00, 1.1111e+03, 4.0000e+02],
        [0.0000e+00, 0.0000e+00, 1.0000e+00]])
        K = torch.tensor(K).cuda().float()

        #make poses to tensor .cuda()  and float    
        poses = torch.tensor(poses).cuda().float()
        #rearrange poses like 'n a b -> n b a' with tumple slice         
